...
- `components/portfolio.tsx` is the shared content and markup source, including project data, cards, galleries, and dialogs. `app/page.tsx` serves the homepage and `app/projects/page.tsx` serves the project collection. Regenerate static editions after editing; avoid editing `index.html` or `projects.html` directly.
...
- `styles/recruiter.css` adds the skills spotlight, technology tape, featured-project carousel, timeline, collection page, and directional reveals.
...
- `scripts/export-static.cjs` compiles the page in memory with the installed TypeScript compiler and renders it with React. CSS and assets remain shared.
- The exporter writes both `index.html` and `projects.html`. Next.js also accepts `/projects.html` and `/index.html` through rewrites, so the same navigation works on Vercel and GitHub Pages.
...
The homepage highlights three selected projects in a manual carousel (swipe, keyboard, arrows, and named selectors). The full nine-project collection lives at `/projects` in Next.js and `projects.html` in the static edition. Screenshot galleries remain inside project dialogs, avoiding nested carousels. Skills are linked to client work, internship experience, and team contributions; the profile pairs factual education and internship milestones with CV/email/phone contact options.

Visual references: [Laracasts](https://laracasts.com/) for clear topic-led cards; [React Bits Logo Loop](https://reactbits.dev/animations/logo-loop), [Scroll Reveal](https://reactbits.dev/text-animations/scroll-reveal), and [Blinking Squares](https://pro.reactbits.dev/docs/components/blinking-squares) for motion direction. The tape, directional reveals, and square field are original CSS/DOM implementations. The Pro page supplied public behavior documentation, not component source; using the exact Pro implementation requires its licensed source. The user-installed `vgpu` dependency is retained, but this CSS effect does not require it. Motion respects the existing effects switch and reduced-motion preference. Technology SVGs are stored locally from [Devicon](https://github.com/devicons/devicon), with its license in `public/assets/tech/LICENSE`.
